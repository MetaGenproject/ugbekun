...
import { Sun, Moon, Mail, Lock, Eye, EyeOff, HelpCircle } from "lucide-react";
    ...
    const handleLogin: SubmitHandler<LoginValues> = async (data) => {
        ...
        try {
            ...
            if (result.success && result.token && result.data?.user) {
                ...
                // Determine redirect path based on role from backend
                const userRole = result.data.user.role;
                const redirectPath = `/${userRole.toLowerCase().replace('_', '-')}`;
                console.log('🔀 Redirecting to:', redirectPath, 'for role:', userRole);
                showPreloader(redirectPath);
            } else {
                console.error('❌ Login failed - missing data:', { success: result.success, hasToken: !!result.token, hasUser: !!result.data?.user });
                toast({
                    variant: "destructive",
                    title: "Login Failed",
