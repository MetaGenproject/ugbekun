...
export default function GradeStudentExamPage() {
    ...
    const exam = exams.find(a => a.id === params.examId);
    const submission = submissions.find(s => s.examId === params.examId && s.studentId === params.studentId);
    ...
    const onSubmit: SubmitHandler<FormValues> = (data) => {
        setSubmissions(prev => prev.map(s => 
            s.id === submission.id ? { ...s, ...data, status: 'graded' } : s
        ));
        
        const allSubmissionsForExam = submissions.filter(s => s.examId === exam.id);
        const allGraded = allSubmissionsForExam.every(s => s.id === submission.id || s.status === 'graded');
        ...
        router.push(`/teacher/grading/exam/${params.examId}`);
    };
