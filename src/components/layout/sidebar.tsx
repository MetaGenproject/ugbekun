...
export function SidebarContent({ schoolData }: { schoolData?: any }) {
    const { plan: contextPlan, isLoading } = usePlan();
    ...
    const currentPlanConfig = plan ? planConfig[plan as keyof typeof planConfig] : planConfig["Starter"];
...
export default function AppSidebar({ schoolData }: { schoolData?: any }) {
    return (
        <div className="hidden xl:flex xl:w-72 xl:flex-col xl:shrink-0">
            <SidebarContent schoolData={schoolData} />
        </div>
    );
}
